...
// Body: { nombor: 1-5, dskp_ids?: string[] (maks 3, tajuk fokus T4/T5) }. Output dalam Rumi.
// Jana SATU soalan Kertas 1 SPM (20 markah, bahagian a-c). Kertas penuh = 5 panggilan serentak dari pelayar.
import { BIDANG_SPM, janaSpmAI, type ContohSpm, type TajukDskp } from '../_shared/gemini.ts';
import { db, semak, semakHadPenjanaan, simpanLog } from '../_shared/db.ts';
import { HttpError, intInRange, postHandler, uuid } from '../_shared/http.ts';
...
postHandler(async ({ body }) => {
  ...
  const soalan = await janaSpmAI({ nombor, tajuk, contoh });
  const log_id = await simpanLog('spm', { nombor, dskp_ids: dskpIds }, soalan);

  return { log_id, soalan };
});
